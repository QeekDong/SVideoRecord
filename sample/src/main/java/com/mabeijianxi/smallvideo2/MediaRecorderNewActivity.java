...
/**
 * Created by win on 2017/9/12.
 */

public class MediaRecorderNewActivity extends Activity
        implements MediaRecorderBaseNew.OnErrorListener, View.OnClickListener, MediaRecorderBaseNew.OnPreparedListener,
        MediaRecorderBaseNew.OnEncodeListener {
     ...
     * SDK视频录制对象
     */
    private MediaRecorderBaseNew mMediaRecorder;
    /**
     * 视频信息
     */
    ...
    public static void goSmallVideoRecorder(Activity context, String overGOActivityName, MediaRecorderConfig mediaRecorderConfig) {
        context.startActivity(new Intent(context, MediaRecorderNewActivity.class).putExtra(OVER_ACTIVITY_NAME, overGOActivityName).putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig));
    }
    ...
    private void initData() {
        ...
        MediaRecorderBaseNew.MAX_FRAME_RATE = mediaRecorderConfig.getMaxFrameRate();
        MediaRecorderBaseNew.NEED_FULL_SCREEN = NEED_FULL_SCREEN;
        MediaRecorderBaseNew.MIN_FRAME_RATE = mediaRecorderConfig.getMinFrameRate();
        MediaRecorderBaseNew.SMALL_VIDEO_HEIGHT = mediaRecorderConfig.getSmallVideoHeight();
        MediaRecorderBaseNew.SMALL_VIDEO_WIDTH = mediaRecorderConfig.getSmallVideoWidth();
        MediaRecorderBaseNew.mVideoBitrate = mediaRecorderConfig.getVideoBitrate();
        MediaRecorderBaseNew.CAPTURE_THUMBNAILS_TIME = mediaRecorderConfig.getCaptureThumbnailsTime();
    ...
    private void loadViews() {
        ...
        mProgressView = (ProgressView) findViewById(com.mabeijianxi.smallvideorecord2.R.id.record_progress);
        mRecordDelete = (CheckedTextView) findViewById(com.mabeijianxi.smallvideorecord2.R.id.record_delete);
...
//        mRecordDelete.setOnClickListener(this);
        ...
        // 是否支持前置摄像头
        if (MediaRecorderBaseNew.isSupportFrontCamera()) {
    ...
    private void initSurfaceView() {
        if (NEED_FULL_SCREEN) {
            ...
            ((RelativeLayout.LayoutParams) mBottomLayout.getLayoutParams()).topMargin = (int) (w / (MediaRecorderBaseNew.SMALL_VIDEO_HEIGHT / (MediaRecorderBaseNew.SMALL_VIDEO_WIDTH * 1.0f)));
            int width = w;
            int height = (int) (w * ((MediaRecorderBaseNew.mSupportedPreviewWidth * 1.0f) / MediaRecorderBaseNew.SMALL_VIDEO_HEIGHT));
            //
            FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mSurfaceView
                    .getLayoutParams();
            lp.width = width;
    ...
    private void initMediaRecorder() {
        mMediaRecorder = new MediaRecorderNativeNew();
...
//                    mRecordDelete.setVisibility(View.GONE);
    ...
    public void onEncodeComplete() {
        ...
        try {
            ...
            intent.putExtra(MediaRecorderNewActivity.OUTPUT_DIRECTORY, mMediaObject.getOutputDirectory());
            intent.putExtra(MediaRecorderNewActivity.VIDEO_URI, mMediaObject.getOutputTempTranscodingVideoPath());
            intent.putExtra(MediaRecorderNewActivity.VIDEO_SCREENSHOT, mMediaObject.getOutputVideoThumbPath());
    ...
    protected void onStop() {
        super.onStop();
        if (mMediaRecorder instanceof MediaRecorderNativeNew) {
            ((MediaRecorderNativeNew) mMediaRecorder).activityStop();
        }
        hideProgress();
